...
import { formatDate, formatPrice, DELIVERY_SLOT_LABELS } from "@/lib/constants";
        ...
        <table className="mt-5 w-full border-collapse text-[11px]">
          <thead>
            <tr className="border-y border-black text-left">
              <th className="w-8 py-1.5 pr-2">Lp.</th>
              <th className="py-1.5 pr-2">Nazwa towaru</th>
              <th className="w-20 py-1.5 pr-2 text-right">Ilość</th>
              <th className="w-16 py-1.5 pr-2">J.m.</th>
              <th className="w-24 py-1.5 pr-2 text-right">Cena</th>
              <th className="w-24 py-1.5 text-right">Wartość</th>
            </tr>
          </thead>
          <tbody>
            {(items ?? []).map((it, idx) => {
